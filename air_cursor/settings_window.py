...
class SettingsWindow(QWidget):
    ...
    def __init__(self, settings: Settings):
        ...
        self._swipe = QCheckBox("Flick sideways → switch windows (Alt+Tab)")
        self._swipe.setChecked(settings.swipe_enabled)
        self._swipe.stateChanged.connect(self._changed)
        self._swipe_up = QCheckBox("Sweep bottom→top → Task View (Win+Tab)")
        self._swipe_up.setChecked(settings.swipe_up_enabled)
        self._swipe_up.stateChanged.connect(self._changed)
        # Higher slider = needs a faster flick (less sensitive / fewer accidents).
        self._swipe_speed = _Slider(1.0, 5.0, settings.swipe_speed)
        self._swipe_speed.valueChangedF.connect(self._changed)
        ...
        gf.addRow("", self._swipe_up)
        gf.addRow("Flick sensitivity (higher = faster needed)", self._swipe_speed)
    ...
    def _changed(self, *_):
        ...
        s.swipe_up_enabled = self._swipe_up.isChecked()
        s.swipe_speed = self._swipe_speed._s.value() / 100.0
